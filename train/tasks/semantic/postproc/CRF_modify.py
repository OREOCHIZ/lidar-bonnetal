...
class CRF_(nn.Module):
    def __init__(self, params, nclasses):
        ...
        # define layers here
        # compat init
        self.compat_kernel_init = np.reshape(np.ones((self.nclasses, self.nclasses)) - np.identity(self.nclasses),
                                             [self.nclasses, self.nclasses, 1, 1])

        # bilateral compatibility matrixes
        self.compat_conv = nn.Conv2d(self.nclasses, self.nclasses, 1)
        self.compat_conv.weight = nn.Parameter(torch.from_numpy(self.compat_kernel_init).float() * self.xyz_coef,
                                               requires_grad=True)

        # locally connected layer for message passing
        self.local_conn_xyz = LocallyConnectedXYZLayer(self.params["lcn_size"]["h"],self.params["lcn_size"]["w"],self.params["xyz_coef"],self.nclasses)
